<div x-data="{openFilters: false}" class="mb-4 px-2 lg:px-6">
        ...
        <div>
            ...
            @else
                <div class="grid grid-cols-2 xs:grid-cols-[repeat(auto-fit,minmax(11rem,auto))] place-items-center gap-3">
                    ...
                    <x-partials._card :product='$accessory' :thumbnail="$accessory->thumbnail" :title="$accessory->title" :url="route('accessories.show',$accessory)">
                        <p class="text-sm leading-4 text-gray-700 font-semibold">
                            ...
                            @endif
                            <span class="@if ($accessory->discount > 0) line-through @endif">{{$settings['currency'] ?? 'Rs '}}. {{ $accessory->amount }}</span>
                        </p>
                    </x-partials._card>
                    @endforeach
                </div>
                ...
                <div class="mt-4">
                    {{ $accessories->links() }} <!-- Pagination Links -->
                </div>
            @endif
        </div>
    </div>
    <div class="max-w-2xl space-y-4">
        @if($accessories)
            @foreach($accessories as $accessory)
                <article class="">
                    <header class="text-xl font-semibold mb-3">{{$accessory->title}}</header>
                    <p class="text-xs text-gray-600">{{$accessory->description}}</p>
                </article>
            @endforeach
        @endif
    </div>
