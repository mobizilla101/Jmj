            ...
            @else
                <div class="grid grid-cols-2 xs:grid-cols-[repeat(auto-fit,minmax(11rem,auto))] place-items-center gap-3">
                    @foreach($secondhandInventories as $secondhandInventory)
                        <x-partials._card :product="$secondhandInventory" :thumbnail="$secondhandInventory->thumbnail" :title="$secondhandInventory->sku->model->model_no" :url="route('refurb.show',$secondhandInventory)">
    ...
    <div class="max-w-2xl space-y-4">
        @if($secondhandInventories)
            @foreach($secondhandInventories as $model)
                <article class="">
                    <header class="text-xl font-semibold mb-3">{{$model->sku->model->model_no}}</header>
