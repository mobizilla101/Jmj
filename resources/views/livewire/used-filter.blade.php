    ...
    <div class="grid grid-cols-1 lg:grid-cols-[15%_1fr] gap-6 px-2 ">
        ...
        >
            ...
            <div class="border border-blue-400 rounded-lg p-2">
                    ...
                    <header class="font-bold">Filter by Storage</header>

                <div
                >
                    @foreach(['64', '128', '256'] as $storageOption)
                    <div class="flex items-center gap-2">
                        <input
                        type="checkbox"
                        x-on:change="(elem)=>$wire.updateStorage(Number(elem.target.value))"
                        :checked="{{ in_array($storageOption, $this->selectedStorage) ? 'true' : 'false' }}"
                        value="{{ $storageOption }}"
                        id="storage-{{ $storageOption }}"
                        class="rounded"
                        />
                        <label for="storage-{{ $storageOption }}">{{ $storageOption }}GB</label>
                    </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="border border-blue-400 rounded-lg p-2">
                    ...
                    <header class="font-bold">Filter by RAM</header>

                <div>
                    @foreach(['4', '8', '16'] as $ramOption)
                    <div class="flex items-center gap-2">
                        <input
                        type="checkbox"
                        x-on:change="(elem)=>$wire.updateRam(Number(elem.target.value))"
                        :checked="{{ in_array($ramOption, $this->selectedRam) ? 'true' : 'false' }}"
                        value="{{ $ramOption }}"
                        id="ram-{{ $ramOption }}"
                        class="rounded"
                        />
                        <label for="ram-{{ $ramOption }}">{{ $ramOption }}GB</label>
                    </div>
                    @endforeach
                </div>
            </div>
        ...
        <!-- Product Listing -->
        <div>
            @if($secondhandInventories->isEmpty())
            ...
            @else
                <div class="grid grid-cols-2 xs:grid-cols-[repeat(auto-fit,minmax(11rem,auto))] place-items-center gap-3">
                    @foreach($secondhandInventories as $secondhandInventory)
                        <x-partials._card :product="$secondhandInventory" :thumbnail="$secondhandInventory->thumbnail" :title="$secondhandInventory->sku->model->model_no" :url="route('used.show',$secondhandInventory)">
                                <p class="text-xs text-gray-600 font-semibold">{{ $secondhandInventory->sku->memory }} RAM |
                                    {{ $secondhandInventory->sku->storage }}GB Storage</p>
                ...
                <div class="mt-4">
                    {{ $secondhandInventories->links() }} <!-- Pagination Links -->
                </div>
            @endif
        </div>
    </div>
    <div class="max-w-2xl space-y-4">
        @if($secondhandInventories)
            @foreach($secondhandInventories as $model)
                <article class="">
                    <header class="text-xl font-semibold mb-3">{{$secondhandInventory->sku->model->model_no}}</header>
