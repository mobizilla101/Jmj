<div x-data="{openFilters: false}" class="mb-4 px-2 lg:px-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-[15%_1fr] gap-6 px-2 ">
        ...
        >
            ...
            <div class="border border-blue-400 rounded-lg p-2">
                <header class="font-bold">Search By</header>
                <div class="rounded border overflow-hidden">
                    <select name="search" class="w-full border-b px-2 py-1" wire:model.live='selectedSearch'>
                        <option value="parts" {{ $this->selectedSearch === 'parts'?'selected':null }}>Parts</option>
                        <option value="model" {{ $this->selectedSearch === 'model'?'selected':null }}>Model</option>
                    </select>

                    <div class="relative">
                        <input type="text" wire:model.lazy='search' class="w-full px-2 py-1"/>
    ...
    <div class="max-w-2xl space-y-4">
        @if($parts )
            @foreach($parts as $part)
                <article class="">
                    <header class="text-xl font-semibold mb-3">{{$part->name}}</header>
