<x-ecom-home-layout>
    <x-slot name="title">
        {{ $secondhandInventory->sku->model->model_no }}
    </x-slot>

    <x-slot name="metaDescription">
        {{$secondhandInventory->description}}
    </x-slot>

    <x-slot name="metaKeywords">
        Refurbed {{$secondhandInventory->sku->model->model_no}}
    </x-slot>

    <x-slot name="metaImage">
        {{asset('storage/'.$secondhandInventory->thumbnail)}}
    </x-slot>
    ...
    <div class="">
        ...
        <article class="my-8 max-w-[90%] xs:max-w-[80%] sm:max-w-[75%] md:max-w-[97%] lg:max-w-[93%] xl:max-w-[80%] mx-auto">
            ...
            <section class="grid grid-rows-[auto_1fr_25.5rem] md:grid-rows-1 md:grid-cols-[22%_40%_1fr] lg:grid-cols-[22%_38%_1fr] gap-4 mb-6">
                <x-key-specs :product="$secondhandInventory->sku->model" />
                <x-product-attachment :product="$secondhandInventory" />
                <x-used.selected :product="$secondhandInventory" />
            </section>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <div x-data="{openMore: false}">
                            ...
                            >
                                {!! str($secondhandInventory->sku->model->specifications)->sanitizeHtml() !!}
                            </div>
