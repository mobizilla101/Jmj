...
class UsedController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('ecom.used.index', compact(
            'filters',
            'brands'
        ));
    }
    ...
    {
        $trending_products = Model::where('published',true)->where('new',true)->where('brand_id',$secondhandInventory->sku->model->brand_id)->where('id', '!=', $secondhandInventory->id)->take(4)->get();
        $related_products = Model::where('published',true)->where('brand_id',$secondhandInventory->sku->model->brand_id)->take(15)->get();

        return view('ecom.used.show',compact(
            'secondhandInventory',
            'trending_products',
            'related_products'
        ));
